  ...
  return (
    ...
    >
        ...
        <nav className="hidden lg:flex items-center space-x-7">
          {NavbarLinks.map((link) => (
            <a
              key={link.id}
              href={link.link}
              className="hover:text-purple-500 hover:underline  hover:transition-all text-lg:"
            >
              {link.name}
            </a>
          ))}
      ...
      <div
        className={`${
          isOpen ? "block" : "hidden"
        } lg:hidden bg-[#801b9c] absolute top-0 left-0 w-full h-screen flex
            flex-col items-center justify-center space-y-8 pt-16`}
      >
        {/* Close Button */}
        <button
